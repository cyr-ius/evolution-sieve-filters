...
- No drag-and-drop rule reordering. Individual rules can be enabled/
  disabled (an "Enabled" checkbox per rule), using the same `if false #
  <test>` convention as Roundcube's managesieve plugin so a disabled
  rule's test isn't lost.
